...
const BookACar = () => {
  const { openModal, toggleModal } = useAddBookingStore();
    ...
    <Modal visible={openModal} animationType="slide" transparent>
      <TouchableWithoutFeedback onPress={() => toggleModal()}>
        <View className="flex-1 items-center justify-center bg-black/50">
          <SafeAreaView className="max-h-[70%] w-11/12 flex-1 rounded-2xl bg-white">
            <KeyboardAvoidingView
              ...
              <ScrollView
                ...
                <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
                  <View className="h-screen gap-4 p-4">
                    {/* DATE */}
                    <Controller
                      name="date"
                      control={control}
                      render={({ field: { value } }) => (
                        <View className="flex gap-4">
                          ...
                          {dateError && (
                            <Text className="text-center text-red-500">{dateError}</Text>
                          )}
                        </View>
                      )}
                    />
